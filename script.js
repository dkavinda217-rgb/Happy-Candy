document.addEventListener('DOMContentLoaded', () => {
    ...
    if (btnReqWhatsapp) {
        btnReqWhatsapp.addEventListener('click', () => {
            ...
            const url = `https://wa.me/94789086123?text=${encodeURIComponent(msg)}`;
            window.open(url, '_blank');
        });
    }
    ...
    function getAIResponse(input) {
        ...
        if (text.includes("contact") || text.includes("phone") || text.includes("number") || text.includes("call")) { // Contact
            return "Call us at +94 78 908 6123 or WhatsApp us! 📞";
        }
    ...
    function triggerSalesAlert() {
        ...
        const alertImg = alertBox.querySelector('img');
        const alertText = alertBox.querySelector('p');
        const alertTime = alertBox.querySelector('small');
        ...
        if (alertImg) alertImg.src = prod.img;
        if (alertText) alertText.innerHTML = `${buyer} just bought <b>${prod.name}</b>!`;
        if (alertTime) alertTime.innerText = `Just now from ${loc}`;
    ...
    document.getElementById('confirm-whatsapp')?.addEventListener('click', () => {
        ...
        msg += `\n*Total:* ${total}\n*Address:* ${addr}\n\nThank you! ✨`;
        window.open(`https://wa.me/94789086123?text=${encodeURIComponent(msg)}`, '_blank');
        cart = [];
        localStorage.setItem('happy_candy_cart', JSON.stringify(cart));
        updateCartUI();
        triggerConfetti();
