document.addEventListener('DOMContentLoaded', () => {
    ...
    function toggleAdminTabs() {
        const isSuper = currentUser && currentUser.email === ADMIN_EMAIL;

        const paymentTab = document.querySelector('.admin-tab[data-tab="payment-tab"]');
        const aboutTab = document.querySelector('.admin-tab[data-tab="about-tab"]');
        const securityTab = document.querySelector('.admin-tab[data-tab="security-tab"]');

        // Only Super Admin (Owner) sees these sensitive tabs
        if (paymentTab) paymentTab.style.display = isSuper ? 'inline-block' : 'none';
        if (aboutTab) aboutTab.style.display = isSuper ? 'inline-block' : 'none';
        if (securityTab) securityTab.style.display = isSuper ? 'inline-block' : 'none';
    }
    ...
    if (aboutForm) {
        aboutForm.addEventListener('submit', (e) => {
            ...
            const aboutConfig = { desc, ownerName, ownerRole };
    ...
    function setLanguage(lang) {
        ...
        if (aboutConfig && aboutConfig.desc) {
            ...
            if (aboutSection) {
                ...
                // If owner info is there
                if (aboutConfig.ownerName) {
                    // Maybe append or update a specific element. For now, let's keep it simple.
                }
            }
        }
    }
    ...
    function getAIResponse(input) {
        ...
        if (text.includes("hi") || text.includes("hello")) {
            return "Hello there! I'm the Happy Candy AI. How can I sweeten your day? 🍬";
        }
        if (text.includes("price") || text.includes("cost") || text.includes("how much")) {
            return "Our regular treats start from Rs. 50! International chocolates start around Rs. 450. You can see full prices in our Gallery section! 💰";
        }
        if (text.includes("delivery") || text.includes("ship")) {
            return "Great news! We offer FREE Delivery for all orders within Sri Lanka! 🚚✨";
        }
        if (text.includes("chocolate")) {
            return "We have amazing international chocolates like Kinder Joy, Ferrero Rocher, KitKat, and Toblerone! Check the Chocolate category! 🍫";
        }
        if (text.includes("cake")) {
            return "Yes! We have Chocolate, Ribbon, Sponge, and Coffee cakes. They are delicious! 🍰";
        }
        if (text.includes("payment") || text.includes("pay")) {
            return "We accept Cash on Delivery, Bank Transfer (DFCC/Sampath), and Online Card Payments! 💳";
        }
        if (text.includes("location") || text.includes("where")) {
            return "We are based in Narammala, but we deliver sweetness all across Sri Lanka! 🇱🇰";
        }
        if (text.includes("contact") || text.includes("phone") || text.includes("number")) {
            return "You can call us at +94 76 849 4653 or email dkavinda217@gmail.com! 📞";
        }

        return "That's a great question! I'm still learning, but you can always chat with our team via WhatsApp for more details! 🍭";
    }
